...
cursor = conn.cursor()
cursor.execute("SELECT user_id FROM users WHERE telegram_id=(%s)", ("1000",))
...
# 		self.cursor.execute("INSERT INTO users(telegram_id) VALUES (%s)", (telegram_id))
...
# 		result = self.cursor.execute("SELECT * FROM records WHERE telegram_id (%s)", (telegram_id))
# 		return result
